...
class ApiService{
  ...
  Future<http.Response> _request(String method, String path, {Map<String,dynamic>? body, bool retry=true}) async{
    String? token = await getAccessToken();
    final uri = Uri.parse('$baseUrl$path');
    final headers = defaultHeaders(token);
    http.Response res;
    if(method=='GET') res = await http.get(uri, headers: headers);
    else if(method=='POST') res = await http.post(uri, headers: headers, body: json.encode(body));
    else if(method=='PATCH') res = await http.patch(uri, headers: headers, body: json.encode(body));
    else if(method=='DELETE') res = await http.delete(uri, headers: headers);
  ...
  Future<bool> _tryRefresh() async{
    ...
    final res = await http.post(uri, headers: {'Content-Type':'application/json'}, body: json.encode({'refresh': refresh}));
  ...
  Future<Map<String,dynamic>> register(String email, String password) async{
    final res = await _request('POST','/api/v1/auth/register', body:{'email':email,'password':password}, retry:false);
    return {'status':res.statusCode, 'body': res.body};
  }
  Future<Map<String,dynamic>> login(String email, String password) async{
    final res = await _request('POST','/api/v1/auth/login', body:{'email':email,'password':password}, retry:false);
    if(res.statusCode==200){
      final j = json.decode(res.body);
      await saveTokens(j['accessToken'], j['refreshToken']);
    }
  ...
  Future<List<dynamic>> getTasks() async{
    final res = await _request('GET','/api/v1/tasks');
  ...
  Future<Map<String,dynamic>> patchTask(String id, Map<String,dynamic> data) async{
  ...
  Future<Map<String,dynamic>> patchCategory(String id, Map<String,dynamic> data) async{
